...
| `uart_gui.py` | **Real-time GUI** — Live serial monitor with 4-channel graphing, stats, and CSV export |
...
```
┌─────────────────────────────────────────────────────────────────────┐
│                    MSP430FR6043 USS MODULE                          │
│                                                                     │
│   ┌──────────────┐    ┌──────────────┐    ┌──────────────────┐     │
│   │  Ultrasonic   │    │   USS Signal  │    │   UART Transmit  │     │
│   │  Transducer   │───▶│  Processing   │───▶│   ($,#,%,! +     │     │
│   │  (UPS & DNS)  │    │  (ToF Calc)   │    │    Hex IEEE754)  │     │
│   └──────────────┘    └──────────────┘    └────────┬─────────┘     │
│                                                     │               │
└─────────────────────────────────────────────────────┼───────────────┘
                                                      │
...
│   │   2. Split by comma  →  [delimiter, hex_string]              │  │
│   │   3. Validate delimiter ($, #, %, !)                         │  │
│   │   4. Decode hex → IEEE 754 float                             │  │
│   │      struct.unpack("f", struct.pack("I", int(hex, 16)))      │  │
│   └──────────────────────────┬───────────────────────────────────┘  │
│                              │                                      │
│                              ▼                                      │
│   ┌──────────────────────────────────────────────────────────────┐  │
│   │                  CHANNEL ROUTING                             │  │
│   │                                                              │  │
│   │   $ ──▶ AbsTof-UPS buffer      # ──▶ AbsTof-DNS buffer      │  │
│   │   % ──▶ DToF buffer            ! ──▶ VFR buffer             │  │
│   └──────────┬───────────────────────────────────┬───────────────┘  │
│              │                                   │                  │
│              ▼                                   ▼                  │
│   ┌────────────────────┐              ┌────────────────────┐       │
│   │  Real-Time Graphs  │              │    CSV File Export  │       │
│   │  (matplotlib)      │              │   (Timestamp,       │       │
│   │  4-channel live    │              │    Channel, Value)  │       │
│   │  rolling plots     │              │                     │       │
│   └────────────────────┘              └────────────────────┘       │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
...
| `uart_gui.py` | **GUI application** — Full-featured Tkinter serial monitor with live matplotlib plots for all 4 channels, real-time stats, raw log viewer, and CSV export capability. |
...
| `tempCodeRunnerFile.python` | Temporary development file (auto-generated by VS Code Code Runner). |
...
4. View live graphs for all 4 channels
5. Optionally click **💾 Save CSV** to export data
...
| 📈 **4-Channel Live Graphs** | Real-time rolling plots (200-point window) for AbsTof-UPS, AbsTof-DNS, DToF, and VFR |
| 📊 **Live Statistics Bar** | Current value display for each channel |
| 📝 **Raw Log Viewer** | Scrollable raw UART data log |
| 💾 **CSV Export** | One-click save to timestamped CSV file |
| 🎨 **Dark Theme** | GitHub-inspired dark UI with color-coded channels |
...
| Channel | Color |
|---|---|
| AbsTof-UPS | 🟦 Cyan (`#00e5ff`) |
| AbsTof-DNS | 🟥 Pink (`#ff4081`) |
| DToF | 🟩 Green (`#69ff47`) |
| VFR | 🟨 Amber (`#ffd740`) |
